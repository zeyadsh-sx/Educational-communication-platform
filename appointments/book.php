...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!verifyCSRFToken($_POST['csrf_token'] ?? '')) {
        ...
        if (empty($professor_id) || empty($appointment_date)) {
            ...
            $stmt = $pdo->prepare("INSERT INTO appointments (student_id, professor_id, appointment_date, date_time, notes, status) VALUES (?, ?, ?, ?, ?, 'pending')");
            $result = $stmt->execute([
                getCurrentUserId(),
                $professor_id,
                $appointment_date,
            ...
            if ($result) {
                $message = 'Appointment booked successfully! You earned 15 points.';
                $messageType = 'success';

                // Award points to student
                awardPoints(getCurrentUserId(), 'book_appointment');
                ...
                $profStmt = $pdo->prepare("SELECT full_name FROM users WHERE id = ?");
                $profStmt->execute([$professor_id]);
                $profName = $profStmt->fetchColumn();

                include_once '../includes/notification_functions.php';
                sendNotification($professor_id, "طالب جديد ({$studentName}) حجز موعد معك في {$appointment_date}");
            } else {
        ...
        <form method="POST" action="" class="appointment-form">
            ...
            <div class="form-group">
                ...
                <select id="professor_id" name="professor_id" class="form-control" required>
                    ...
                    <?php
                    $professorsStmt = $pdo->prepare("SELECT id, full_name, email FROM users WHERE user_type = 'professor' ORDER BY full_name");
                    $professorsStmt->execute();
                    $professors = $professorsStmt->fetchAll();

                    foreach ($professors as $professor): ?>
                        <option value="<?php echo $professor['id']; ?>" <?php echo (isset($_POST['professor_id']) && $_POST['professor_id'] == $professor['id']) ? 'selected' : ''; ?>>
                            <?php echo htmlspecialchars($professor['full_name']); ?> (<?php echo htmlspecialchars($professor['email']); ?>)
                        </option>
                    <?php endforeach; ?>
                </select>
            </div>
